...
export default function POSLogin() {
  const [activeTab, setActiveTab] = useState<"pin" | "password" | "activate">("pin");
  ...
  const [activationPwd, setActivationPwd] = useState("");
  ...
  useEffect(() => {
    ...
    } else if (sessionStatus?.isActivated && activeTab === "activate") {
      setActiveTab("pin");
    }
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white">
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-slate-800">
              {!isActivated ? "Terminal Activation" : "Staff Authentication"}
            </h2>
            <p className="text-slate-500 text-sm">
              {!isActivated 
                ? "Enter your organization credentials to link this device." 
                : sessionStatus?.locationName || "Store Context Active"}
            </p>
          </div>

          {/* Tabs - Only show when activated */}
          {isActivated && (
