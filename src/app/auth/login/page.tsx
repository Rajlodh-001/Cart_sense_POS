...
import { login, clearError } from "@/app/auth/authSlice";
...
export default function POSLogin() {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    if (activeTab === "password" && email && password) {
      ...
      // PIN login — not yet supported by backend
      // For now, show a message
      if (pin.length < 4) return;
      alert(
        "PIN login is not yet supported by the backend. Please use Manager Login.",
      );
    }
  };
